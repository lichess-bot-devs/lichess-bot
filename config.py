...
def load_config(config_file):
    with open(config_file) as stream:
        ...
        engine_sections = [["dir", str, "´dir´ must be a string wrapped in quotes."],
                           ...
                           ["polyglot_move_selection", str, "´polyglot_move_selection´ must be a string wrapped in quotes."]]
        ...
        if CONFIG["engine"]["polyglot"] == True:
            polyglot_move_values = ["random_move", "random_weight", "best_move"]
            if CONFIG["engine"]["polyglot_move_selection"] not in polyglot_move_values:
                raise Exception("´polyglot_move_selection´ must be one of these values: %s" % ", ".join(polyglot_move_values))
