    ...
    with open(config_file) as stream:
        ...
        for section in sections:
            if section[0] not in CONFIG:
                raise Exception(
                    f"Your config.yml does not have required section `{section[0]}`."
                )
        ...
        for subsection in engine_sections:
            if subsection[0] not in CONFIG["engine"]:
                raise Exception(
                    f"Your config.yml does not have required `engine` subsection `{subsection}`."
                )
        ...
        if (
            not os.access(engine, os.X_OK)
            and CONFIG["engine"]["protocol"] != "homemade"
        ):
            raise Exception(
                "The engine %s doesn't have execute (x) permission. Try: chmod +x %s"
                % (engine, engine)
            )
