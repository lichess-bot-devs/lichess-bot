...
class EngineWrapper:
    ...
    def offer_draw_or_resign(self, result, board):
        ...
        draw_score_range = self.draw_or_resign.get("offer_draw_score")
        draw_max_piece_count = self.draw_or_resign.get("offer_draw_pieces")
        ...
        resign_score = self.draw_or_resign.get("resign_score")
    ...
    for book in books:
        with chess.polyglot.open_reader(book) as reader:
            try:
                ...
                min_weight = polyglot_cfg.get("min_weight")
...
def get_lichess_cloud_move(li, board, game, lichess_cloud_cfg):
    wb = "w" if board.turn == chess.WHITE else "b"
    time_left = game.state[f"{wb}time"]
    min_time = lichess_cloud_cfg.get("min_time") * 1000
    use_lichess_cloud = lichess_cloud_cfg.get("enabled")
